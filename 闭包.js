//	对闭包的概念及原理模糊不清，一直以来都是以通俗的外层函数包裹内层来欺骗自己。这种说法并没有对与错，只是如果说出更好更低层的东西，逼格会提升好几个档次.
//	闭包是JavaScript两个核心技术之一(异步和闭包),在面试以及实际应用当中，我们都离不开它们，甚至可以说它们是衡量js工程师实力的一个重要指标。
//	下面罗列闭包的几个常见问题，从回答问题的角度来理解和定义闭包。
//	
//	主要问题：
//	1.什么是闭包? 
//	2.闭包的原理可不可以说一下？ 
...
//	
//	可见，它们各有各自的定义，但要说明的意思大同小异。
//	
//	因此想要深入的理解，从词法作用域到作用域链的概念再到闭包的形成做了一次总的梳理。
//	
//	下面让抛开这些抽象而又晦涩难懂的表述，从头开始理解，内化最后总结出自己的一段关于闭包的句子。我想这对面试以及充实开发者自身的理论知识非常有帮助。
//	
//	闭包的构成
//	
//	词法作用域
//	
//	要理解词法作用域，我们不得不说起JS的编译阶段，JS是弱类型语言，所谓弱类型是指不用预定义变量的储存类型，并不能完全概括JS或与其他语言的区别，在这里可以引用黄皮书(《你不知道的javascript》)上的给出的解释编译语言。
//	
//	编译语言
...
//	
//	如果词法单元生成器拿不准当前词法单元是否为独立的，就进入词法分析，否则就进入分词阶段。
//	这就是理解词法作用域及其名称来历的基础。
//	
...
//	    }  
//	   foo();
//	再来看上面这个简单的例子，可以先思考一下，每个执行环境下的变量对象都是什么？ 这两个函数它们的变量对象分别都是什么？
//	
//	以fun为例，当调用它时，会创建一个包含 arguments，a，b的活动对象，对于函数而言，在执行的最开始阶段它的活动对象里只包含一个变量，即arguments(当执行流进入，再创建其他的活动对象)。
//	
//	在活动对象中，它依然表示当前参数集合。对于函数的活动对象，我们可以想象成两部分，一个是固定的arguments对象，另一部分是函数中的局部变量。而在此例中，a和b都被算入是局部变量中，即便a已经包含在了arguments中，但他还是属于。
//	
//	有没有发现在环境栈中，所有的执行环境都可以组成相对应的作用域链。在环境栈中可以非常直观的拼接成一个相对作用域链。
//	
//	
//	下面大致说下这段代码的执行流程：
//	
...
//	6.作用域链其实就是引用了当前执行环境的变量对象的指针列表，它只是引用，但不是包含。，因为它的形状像链条，它的执行过程也非常符合，所以我们都称之为作用域链，而当我们弄懂了这其7.中的奥秘,就可以抛开这种形式上的束缚，从原理上出发。
//	
//	到这里应该知道：
//	
//	什么是作用域链。
...
//	使用闭包
//	
//	涉及的理论充实了，那么现在就要使用它了。 先上几个最简单的计数器例子：
//	
...
//	    }    
//	 }
//	它们都有一个小特点，就是实现的过程都返回一个函数对象，返回的函数中带有对外部变量的引用。
//	
//	为什么非要返回一个函数呢 ? 
...
//	    })(i);
//	
//	上例中两段代码，第一个我们在面试过程中一定碰到过，这是一个异步的问题，它不是一个闭包，但可以通过闭包的方式解决。
//	
//	第二段代码会输出 1- 5 ，因为每循环一次回调中都引用了参数i(也就是活动对象)，而在上一个循环中，每个回调引用的都是一个变量i，其实还可以用其他更简便的方法来解决。
//	
//	       for (let i = 1; i <= 5; i++) {
//	               setTimeout( function timer() {
//	                          console.log(i);  
...
//	            var s = "javascript";
//	    }(window,document))
//	有些说法把这种方式称之为闭包，并说闭包可以避免全局污染，首先在这里应该有一个自己的答案，以上这个例子是一个闭包吗？
//	
...
//	3.你是怎样使用闭包的？
//	
//	使用闭包的场景有很多,可以说在js中闭包其实就是函数式的一个重要基础,举个不完全函数的栗子.
//	
...
//	var k = n(8,9,10);
//	delete n;
//	上面这个栗子，就是保留对 fun函数的活动对象(arguments[])，在日常开发中还有更复杂的情况，这需要很多函数块，到那个时候，才能显出闭包的真正威力